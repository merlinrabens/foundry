...
# Priority: 1) credentials file 2) Keychain 3) token cache written by spawn/respawn
...
if [ -z "$_oauth_token" ] && [ -f "$HOME/.claude/.foundry-token" ]; then
  _oauth_token=$(cat "$HOME/.claude/.foundry-token" 2>/dev/null || true)
fi
