...
Three-backend coding agent system with OpenClaw as orchestrator. Spawns isolated agents in git worktrees, monitors them with zero AI tokens, auto-respawns on failure, and tracks patterns for model routing. Uses ACP (Agent Client Protocol) for structured agent management with PID-based process lifecycle. OpenClaw picks the best agent per task via smart routing.
...
│                                                                     │
│  ┌──────────┐     ┌──────────────────────────────────────────────┐  │
│  │ Foundry  │────▶│ foundry CLI (bash dispatcher, ~53 lines)     │  │
...
lib/                      Pure-function modules (12 files, ~900 lines)
  ...
  smart_routing.bash        Smart agent selection (patterns + heuristics)
...
1. `foundry spawn <repo> <spec> [model]` creates an isolated git worktree, launches a coding agent (Codex, Claude, or Gemini) via ACP as a background process with PID tracking. `foundry orchestrate` adds Smart routing layer on top.
...
### Three Backends + Foundry Routing
...
- `openclaw` — Smart routing (picks best agent automatically)
- `openclaw:codex` / `openclaw:claude` / `openclaw:gemini` — Smart routing with hint

**Decision tree:** Design task? → Gemini. Simple frontend? → Claude. Everything else? → Codex. Or use `openclaw` to let Foundry decide.

### Smart Routing (`openclaw` / `orchestrate`)

Foundry is the orchestrator — it picks the best agent per task using:
...
# Sets: MODEL_OUT="auto", AGENT_BACKEND_OUT="auto"

detect_model_backend "openclaw:codex"
# Sets: MODEL_OUT="codex", AGENT_BACKEND_OUT="auto"
```

### smart_routing.bash

Smart agent selection. Resolves the `auto` meta-backend into a concrete agent.

```bash
source lib/smart_routing.bash
...
- `R2_BUCKET` (optional, defaults to `your-project-assets`)
- `CDN_BASE_URL` (optional, defaults to `https://assets.your-project.com`)
...
foundry orchestrate <repo> <spec|task> [hint]     # Foundry picks best agent + spawns (JSON output)
...
```json
{
  "id": "your-repo-12-agent-friendly-linting",
  "repo": "your-repo",
  "repoPath": "/Users/me/projects/org/your-repo",
